...
public class CheatFrame extends JFrame {
	 ...
	 */
	public CheatFrame(User currentUser, SQLInteraction sqlInteraction) {
		setResizable(false);
		if (currentUser != null) {
	...
	public CheatFrame(User currentUser, BankAccount currentBankAccount, SQLInteraction sqlInteraction) {
		if (currentUser != null) {
			this.firstName = currentUser.getFirstName();
			this.lastName = currentUser.getLastName();
			this.address = currentUser.getAddress();
	...
	private void initComponentsUser() {
		...
		JLabel frameLabel = new JLabel("GODMODE: MONEY EVERYWHERE !!");
		...
		customerAccountInput.setEditable(true);
