	...
	public Vector<User> getUser(int role){
		...
		try {
			...
			while(rs.next()){
				switch(role) {
				case customerRole:
					vectUser.add(new Customer(rs.getInt("id"), rs.getString("address"), rs.getString("firstName"), rs.getString("lastName"), rs.getString("mail"), rs.getString("password"),this.sqlInteraction,rs.getString("counceilor")));
				break;
				case bankerRole:
